...
class _ViewApplicationState extends State<ViewApplication> {
  ...
  getapplication() async {
    ...
    try {
      var res = await api.GetApplicationsofStu(USER_ID.toString());
      print("res $res");
  ...
  update(String id, String status) async {
    setState(() {
      isLoading = true;
    });
    try {
      var res = await api.UpdateapplicationStatus(id, status);
      print("res $res");
      if (res['codeStatus'] == true) {
        getapplication();
      } else {
